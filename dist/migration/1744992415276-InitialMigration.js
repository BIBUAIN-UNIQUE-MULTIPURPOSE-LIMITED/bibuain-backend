...
exports.InitialMigration1744992415276 = void 0;
class InitialMigration1744992415276 {
    constructor() {
        this.name = 'InitialMigration1744992415276';
    }
    up(queryRunner) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield queryRunner.query(`CREATE TYPE "public"."trades_feedback_enum" AS ENUM('positive', 'negative')`);
            yield queryRunner.query(`CREATE TABLE "trades" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "tradeHash" character varying(100) NOT NULL, "accountId" character varying(100) NOT NULL, "platform" "public"."trades_platform_enum" NOT NULL, "status" "public"."trades_status_enum" NOT NULL DEFAULT 'pending', "tradeStatus" character varying NOT NULL, "tradeDetails" json, "tradeChat" json, "feedback" "public"."trades_feedback_enum", "amount" numeric(20,2) NOT NULL, "cryptoAmountRequested" numeric(20,8) NOT NULL, "cryptoAmountTotal" numeric(20,8) NOT NULL, "feeCryptoAmount" numeric(20,8) NOT NULL, "flagged" boolean NOT NULL DEFAULT false, "isEscalated" boolean NOT NULL DEFAULT false, "escalationReason" text, "escalated_by_id" uuid, "assigned_cc_agent_id" uuid, "feePercentage" numeric(10,2) NOT NULL, "sourceId" character varying(100) NOT NULL, "responderUsername" character varying(100) NOT NULL, "ownerUsername" character varying(100) NOT NULL, "paymentMethod" character varying(100) NOT NULL, "locationIso" character varying(2), "fiatCurrency" character varying(3) NOT NULL, "cryptoCurrencyCode" character varying(10) NOT NULL, "isActiveOffer" boolean NOT NULL DEFAULT false, "offerHash" character varying(100), "margin" numeric(10,2), "dollarRate" numeric(20,2), "btcRate" numeric(20,8), "btcAmount" numeric(20,8), "assigned_payer_id" uuid, "assignedAt" TIMESTAMP, "completedAt" TIMESTAMP, "btcNgnRate" double precision, "usdtNgnRate" double precision, "notes" character varying(255), "platformMetadata" jsonb, "activityLog" jsonb, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), "parent_trade_id" uuid, CONSTRAINT "UQ_TRADE_HASH" UNIQUE ("tradeHash"), CONSTRAINT "PK_c6d7c36a837411ba5194dc58595" PRIMARY KEY ("id")); COMMENT ON COLUMN "trades"."accountId" IS 'External account identifier for the trade'`);
            ...
            yield queryRunner.query(`CREATE TABLE "rates" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "sellingPrice" numeric(50,8) NOT NULL, "usdtNgnRate" numeric(50,8) NOT NULL, "marketcap" numeric(20,2) NOT NULL DEFAULT '0', "btcngnrate" numeric(20,4) NOT NULL DEFAULT '0', "platformRates" jsonb, "paxfulRate" numeric(50,8), "noonesRate" numeric(50,8), "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_2c804ed4019b80ce48eedba5cec" PRIMARY KEY ("id"))`);
            ...
            yield queryRunner.query(`ALTER TABLE "trades" ADD CONSTRAINT "FK_19214e66bdc060249a3b002c706" FOREIGN KEY ("parent_trade_id") REFERENCES "trades"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
            yield queryRunner.query(`ALTER TABLE "trades" ADD CONSTRAINT "FK_de5663a26e8e69b83e95e9436ed" FOREIGN KEY ("escalated_by_id") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
            yield queryRunner.query(`ALTER TABLE "trades" ADD CONSTRAINT "FK_06f435319092f4ba8677b09ac48" FOREIGN KEY ("assigned_cc_agent_id") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    down(queryRunner) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield queryRunner.query(`ALTER TABLE "trades" DROP CONSTRAINT "FK_06f435319092f4ba8677b09ac48"`);
            yield queryRunner.query(`ALTER TABLE "trades" DROP CONSTRAINT "FK_de5663a26e8e69b83e95e9436ed"`);
            yield queryRunner.query(`ALTER TABLE "trades" DROP CONSTRAINT "FK_19214e66bdc060249a3b002c706"`);
            ...
            yield queryRunner.query(`DROP TYPE "public"."trades_feedback_enum"`);
...
exports.InitialMigration1744992415276 = InitialMigration1744992415276;
