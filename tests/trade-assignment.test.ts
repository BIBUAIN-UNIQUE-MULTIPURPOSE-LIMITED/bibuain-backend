...
jest.mock("../src/controllers/tradeController", () => {
  const originalModule = jest.requireActual(
    "../src/controllers/tradeController",
  );
  return {
    ...originalModule,
    initializePlatformServices: jest
      .fn()
      .mockResolvedValue(mockPlatformServices),
  };
});
...
describe("Trade Controller Tests", () => {
  ...
  beforeEach(() => {
    ...
    (tradeController.initializePlatformServices as jest.Mock).mockResolvedValue(
      mockPlatformServices,
    );
    ...
    mockDbConnect.getRepository = jest
      .fn()
  ...
  describe("Additional Trade Controller Scenarios", () => {
    ...
    it("4. should escalate trade successfully", async () => {
      ...
      const req = createMockRequest(
        { tradeId: "trade1" },
        { reason: "Payment dispute", escalatedById: "payer1" },
      );
      const res = createMockResponse();
      const next = createMockNext();

      await tradeController.escalateTrade(
        req as Request,
        res as Response,
        next,
      );
      ...
      expect(mockIo.emit).toHaveBeenCalledWith("tradeEscalated", {
        tradeId: "trade1",
      });
    ...
    it("5. should prevent escalated trade from being reassigned to payer", async () => {
      ...
      mockTradeRepo.find.mockResolvedValue([
        { ...mockTrades[2], isEscalated: true, status: TradeStatus.ESCALATED },
      ]);
    ...
    it("6. should mark trade as paid", async () => {
      ...
      await tradeController.markTradeAsPaid(
        req as Request,
        res as Response,
        next,
      );
      ...
      expect(
        mockPlatformServices.paxful[0].markTradeAsPaid,
    ...
    it("7. should prevent completed trade from being reassigned", async () => {
      ...
      mockTradeRepo.find.mockResolvedValue([
        { ...mockTrades[0], status: TradeStatus.COMPLETED },
      ]);
      mockUserRepo.find.mockResolvedValue([mockUsers[0]]);
      mockShiftRepo.find.mockResolvedValue([mockShifts[0]]);
      mockQueryRunner.manager.find
      ...
      await tradeController.reassignTrade(
        req as Request,
        res as Response,
        next,
      );
  ...
  describe("escalateTrade", () => {
    it("should escalate trade successfully", async () => {
      ...
      const req = createMockRequest(
        { tradeId: "trade1" },
        { reason: "Payment issue", escalatedById: "payer1" },
      );
      const res = createMockResponse();
      const next = createMockNext();
  ...
  describe("assignLiveTradesInternal", () => {
    ...
    it("should handle database errors", async () => {
      ...
      await expect(tradeController.assignLiveTradesInternal()).rejects.toThrow(
        "Database error",
      );
      expect(mockQueryRunner.rollbackTransaction).toHaveBeenCalled();
    });
