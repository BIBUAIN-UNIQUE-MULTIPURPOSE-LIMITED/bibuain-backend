...
router.get("/", roleAuth([UserType.ADMIN, UserType.RATER]), getAllBanks);
router.get("/free", roleAuth([UserType.ADMIN, UserType.RATER]), getFreeBanks);
router.get(
  "/funded",
...
router.delete("/:id", roleAuth([UserType.ADMIN, UserType.RATER]), deleteBank);
...
router.post("/use/:id", roleAuth([UserType.PAYER]), useBank);

// Admin cron route (optional, protected)
router.post("/refresh", roleAuth([UserType.ADMIN]), async (req, res, next) => {
  try {
